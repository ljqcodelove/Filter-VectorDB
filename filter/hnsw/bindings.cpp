...
template<typename dist_t = int32_t, typename data_t = uint8_t>
class Index {
 public:
    ...
    hnswlib::SpaceInterface<int32_t>* l2space;
...
//    void set_only_data_level0_memory_(const Index &index){
//        appr_alg->only_data_level0_memory_new_ = index.appr_alg->only_data_level0_memory_new_;
//    }
    ...
    void normalize_vector(uint8_t* data, uint8_t* norm_array) {
        int32_t norm = 0.0;
        for (int i = 0; i < dim; i++)
            norm += data[i] * data[i];
        norm =  (int)(sqrtf(norm));
    ...
    void setFilterLabels(py::object input, int id, int size) {
        py::array_t < dist_t, py::array::c_style | py::array::forcecast > items(input);

        appr_alg->addFilterLabels((int*) items.data(), (size_t)id, (size_t) size);
    }
    ...
    void addItems(py::object input, py::object ids_ = py::none(), int num_threads = -1, bool replace_deleted = false) {
...

        {
            ...
            if (normalize == false) {
                ...
                std::vector<uint8_t> norm_array(num_threads * dim);
                ParallelFor(start, rows, num_threads, [&](size_t row, size_t threadId) {
                    // normalize vector:
...
//    void addOnlyDataItems(py::object input, py::object ids_ = py::none(), int num_threads = -1) {
//        py::array_t < data_t, py::array::c_style | py::array::forcecast > items(input);
//        auto buffer = items.request();
//        if (num_threads <= 0)
//            num_threads = num_threads_default;
...
//                vector_data = norm_array.data();
//            }
//            appr_alg->addOnlyDataPoint((void*)vector_data, (size_t)id);
//            start = 1;
//            ep_added = true;
//        }
//
...
//        ParallelFor(start, rows, num_threads, [&](size_t row, size_t threadId) {
//            size_t id = ids.size() ? ids.at(row) : (cur_l + row);
//            appr_alg->addOnlyDataPoint((void*)items.data(row), (size_t)id);
//        });
//
//        cur_l += rows;
//    }
//
//    void addItemsNew(py::object input, py::object ids_ = py::none(), int num_threads = -1, bool replace_deleted = false) {
//        py::array_t < data_t, py::array::c_style | py::array::forcecast > items(input);
//        auto buffer = items.request();
//        if (num_threads <= 0)
//            num_threads = num_threads_default;
...
//                    vector_data = norm_array.data();
//                }
//                appr_alg->addWithoutDataPoint((void*)vector_data, (size_t)id, replace_deleted);
//                start = 1;
//                ep_added = true;
//            }
//
//            printf("fun for test 2\n");
//            py::gil_scoped_release l;
//            if (normalize == false) {
//                ParallelFor(start, rows, num_threads, [&](size_t row, size_t threadId) {
//                    size_t id = ids.size() ? ids.at(row) : (cur_l + row);
//                    appr_alg->addWithoutDataPoint((void*)items.data(row), (size_t)id, replace_deleted);
//                    });
//            } else {
//                std::vector<uint8_t> norm_array(num_threads * dim);
//                ParallelFor(start, rows, num_threads, [&](size_t row, size_t threadId) {
...
//
//                    size_t id = ids.size() ? ids.at(row) : (cur_l + row);
//                    appr_alg->addWithoutDataPoint((void*)(norm_array.data() + start_idx), (size_t)id, replace_deleted);
//                    });
//            }
//            cur_l += rows;
//        }
//        printf("fun for test 3\n");
//
//        std::vector<size_t> ids__;
//        swap(ids, ids__);
//    }
//
    ...
    py::dict getIndexParams() const { /* WARNING: Index::getAnnData is not thread-safe with Index::addItems */
        auto params = py::dict(
            "ser_version"_a = py::int_(Index<>::ser_version),  // serialization version
            "space"_a = space_name,
            "dim"_a = dim,
    ...
    static Index<>* createFromParams(const py::dict d) {
        ...
        assert_true(((int)py::int_(Index<>::ser_version)) >= d["ser_version"].cast<int>(), "Invalid serialization version!");
    ...
    py::object knnQuery_return_numpy_binary_search(
        py::object input,
        ...
        std::vector<size_t> ids = get_input_ids_and_check_shapes(ids_, lenOfRows);

        {
            ...
            if (normalize == false) {
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                    std::priority_queue<std::pair<dist_t, hnswlib::tableint >> enterpoint_node_queue = appr_alg->searchKnnReturnEpId((void*)items.data(row), 1);

                    if(enterpoint_node_queue.empty())
                        throw std::runtime_error(
                            "Cannot find the enterpoint_node");
                    hnswlib::tableint enterpoint_node_now = enterpoint_node_queue.top().second;

//                    hnswlib::tableint enterpoint_node_now = 0;

                    int l = 2, r = 3, ans = 3;
                    while (l <= r) {
                        int mid = l + r >> 1;
                        std::vector<hnswlib::tableint> tmpCandidateSet = appr_alg->bfsByDannyReturnVector(enterpoint_node_now, ids, (void*)items.data(row), 0, mid, w1, w2);
                        if (tmpCandidateSet.size() >= k) {
                            ans = mid;
                            break;
                        } else {
                            l = mid + 1;
                    ...
                    std::vector<hnswlib::tableint> tmpCandidateSet = appr_alg->bfsByDannyReturnVector(enterpoint_node_now, ids, (void*)items.data(row), 0, ans, w1, w2);
//                    std::queue<std::pair<int, hnswlib::tableint>> tmpCandidateSet = appr_alg->bfsByDanny(enterpoint_node_now, (void*)items.data(row), 0, ans, w1, w2);
//                    printf("ans is %d, and size is %d\n", ans, tmpCandidateSet.size());
                    std::priority_queue<std::pair<dist_t, hnswlib::labeltype >> result = appr_alg->searchKnnByBruteForceUsingVector((void*)items.data(row), k, tmpCandidateSet);
//                    std::priority_queue<std::pair<dist_t, hnswlib::labeltype >> result = appr_alg->searchKnn(
//                        (void*)items.data(row), k, p_idFilter);
                    while (result.size() < k) {
                        result.emplace(2000000000, -1);
                    }
                    if (result.size() != k)
                        throw std::runtime_error(
...
//                std::vector<uint8_t> norm_array(num_threads * features);
//                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
//                    uint8_t* data = (uint8_t*)items.data(row);
//
//                    size_t start_idx = threadId * dim;
//                    normalize_vector((uint8_t*)items.data(row), (norm_array.data() + start_idx));
//
...
//    py::object knnQuery_return_numpy_new(
//        py::object input,
//        size_t k = 1,
//        int num_threads = -1,
//        const std::function<bool(hnswlib::labeltype)>& filter = nullptr) {
...
//            if (normalize == false) {
//                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
//                    std::priority_queue<std::pair<dist_t, hnswlib::labeltype >> result = appr_alg->searchWithoutDataKnn(
//                        (void*)items.data(row), k, p_idFilter);
//                    if (result.size() != k)
//                        throw std::runtime_error(
//                            "Cannot return the results in a contiguous 2D array. Probably ef or M is too small");
...
PYBIND11_PLUGIN(hnswlib) {
        ...
        .def("knn_query_one_stage",
            &Index<>::knnQuery_return_numpy_binary_search,
            py::arg("data"),
            py::arg("ids") = py::none(),
            py::arg("len_of_rows") = -1,
            py::arg("k") = 1,
            py::arg("num_threads") = -1,
            py::arg("filter") = py::none(),
            py::arg("w1") = -1,
            py::arg("w2") = -1)
//        .def("knn_query_new",
//            &Index<>::knnQuery_return_numpy_new,
//            py::arg("data"),
//            py::arg("k") = 1,
//            py::arg("num_threads") = -1,
//            py::arg("filter") = py::none())
        .def("set_filter_labels",
            &Index<>::setFilterLabels,
            py::arg("data"),
            py::arg("id") = -1,
            py::arg("size") = 0)
        .def("add_items",
...
//            py::arg("num_threads") = -1,
//            py::arg("replace_deleted") = false)
//        .def("add_only_data_items",
//            &Index<>::addOnlyDataItems,
//            py::arg("data"),
//            py::arg("ids") = py::none(),
//            py::arg("num_threads") = -1)
...
//        .def("set_only_data_level0_memory", &Index<>::set_only_data_level0_memory_, py::arg("index"))
        .def("load_index",
            &Index<>::loadIndex,
...
//        .def("clear_lock", &Index<>::clearLock)
        ...
        .def_property_readonly("max_elements", [](const Index<> & index) {
            return index.index_inited ? index.appr_alg->max_elements_ : 0;
        })
        .def_property_readonly("element_count", [](const Index<> & index) {
            return index.index_inited ? (size_t)index.appr_alg->cur_element_count : 0;
        })
        .def_property_readonly("ef_construction", [](const Index<> & index) {
          return index.index_inited ? index.appr_alg->ef_construction_ : 0;
        })
        .def_property_readonly("M",  [](const Index<> & index) {
